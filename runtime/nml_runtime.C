...
bool show_progress = false; //TODO: make this compile time switch?
...
Counter unappliedB_allocation("UB");
Counter unappliedC_allocation("UC");
...
SiCont* makeSiCont(char* name, Ncode code, unsigned frame_size) {
...
void* heap_alloc(Counter& counter, size_t size); //forward
...
void SetBrokenHeart(Hob* before, Hob* after) {
  //placement new: overwrite old object with BrokenHeart
  new (before) BrokenHeart(after);
}
...
//two LSM bits are zero & value>255; property of pointer
bool isTaggedPointer(xint raw) { return (raw & ~255) && !(raw & 3); }
...
xint TagInt(xint i) {
  //two MSB bit are the same (i.e. we have a 31bit int)
  //TODO: why do we care to assert this?
  assert ((static_cast<unsigned>(i) >> 30 == 0)
          || (static_cast<unsigned>(i) >> 30 == 3));
  return (i << 1) | 1; // shift & tag
}

xint TagUnsigned(unsigned u) {
  //two MSB bit are zero (i.e. we have a 30bit unsigned)
  //TODO: why do we care to assert this?
  assert(!(u>>30));
...
void top_evacuate(Nword& w) {
  ...
  // TODO: can we avoid repeatedly setting this broken heart?
  SetBrokenHeart(before,after);
  w = after;
}
...
//----------------------------------------------------------------------
//INDEX: HeapPointer: where allocation occurs. The heap grows upwards.
//----------------------------------------------------------------------

char* HeapPointer = TheInitHeap;
...
void gc() {
  ...
  // evacuate roots; depends on whether we are in closure/continuation/handler
...
void* stack_alloc(size_t size) {
  assert(size%4==0);
  StackPointer -= size;
  assert(StackPointer>=TheStack); //stack-overflow
  if (StackPointer < StackLowWater) { StackLowWater = StackPointer; } // stats
  return StackPointer;
}
...
void  maybe_gc() {
  ...
  bool need2gc =
    HeapPointer + MaxAlloctaionWhichMightOccurBeforeNextJump > currentSpace.top;
  if (need2gc) { gc(); }
}
...
//TODO: reinstate file-static declaration for "the_unit"
...
// TODO: (old) Would this be better recoded to dispatch?
...
public:
  ...
  bool equalTo(Nword v) {
    return tag() == getTag(v) && equalWord(word,getCon(v));
  }
  unsigned bytes() { return HobBytes(0); }
...
Nword makeCon0(unsigned tag) {
  return Nword::fromUnsigned(tag);
}

Nword makeCon1(unsigned tag, Nword w) {
  switch(tag) {
#define TAG(T) case T: { \
                 return new (con1_data_allocation) Value_Con1_TAG<T>(w); \
               }
    TAG (0); TAG (1); TAG (2); TAG (3); TAG (4);
    TAG (5); TAG (6); TAG (7); TAG (8); TAG(9);
    TAG(10); TAG(11); TAG(12); TAG(13); TAG(14);
    TAG(15); TAG(16); TAG(17); TAG(18); TAG(19);
  }
...
public:
  ...
  unsigned tag;
  Nword word;
  Value_ExCon1(std::string name_, unsigned tag_, Nword word_)
    : name(name_), tag(tag_), word(word_) {}
  std::string what() { return "ExCon1-"+name; }
  bool equalTo(Nword v) {
    return tag == getExTag(v) && equalWord(word,getExCon(v));
  }
  unsigned bytes() { return HobBytes(0); }
  Hob* evacuate() {
    return new (EvacuationAllocation) Value_ExCon1(name,tag,word);
  }
  void scavenge() { ::top_evacuate(word); }
};
...
std::string getExName(Nword w) {
  ...
  if (Value_ExCon1* x = dynamic_cast<Value_ExCon1*>(getPointer(w))) {
    return x->name;
  }
  if (Closure* c = dynamic_cast<Closure*>(getPointer(w))) {
    if (Value_ExCon0* x =
        dynamic_cast<Value_ExCon0*>(getPointer(c->frameElem(0)))) {
      return x->name;
    }
  }
...
class Value_Vector : public Hob {
  ...
  Nword* _words;
public:
  // TODO: old comment (where is this allocated?) - why is this important?
  // also the case for Array below
  Value_Vector(unsigned n): _n(n), _words(new Nword[_n]) {}
...
public:
  ...
  bool equalTo(Nword w2) {
    return (this == getPointer(w2)); // pointer-equality for refs.
  }
  unsigned bytes() { return HobBytes(0); }
...
class Value_Array : public Hob {
  ...
  Nword* _words;
public:
  Value_Array(unsigned n): _n(n), _words(new Nword[_n]) {}
...
public:
  ...
  static void* operator new(size_t size, unsigned extra_words) {
    return stack_alloc(size+extra_words*sizeof(Nword));
  }
  Nword& frameElem(unsigned i) {
    assert(i<si->frame_size);
    return words[i];
...
Ncode RaiseWith(Nword res) {
  ...
  if (show_progress) {
    ...
    cout << "**Sliding: " << (reinterpret_cast<char*>(CH) - StackPointer)
         << endl;
  }
...
void PushHandler(SiCont* si) {
  if (show_progress) { cout << "**Pushhandler: " << si->name << endl; }
  TheCurrentHandler = new (si->frame_size) Handler(si,TheCurrentHandler);
  PushContinuation(&SiPopHandler);
}
...
void ShiftUpArgs_upto(unsigned shift, unsigned n) {
  if (n>0) {
    GetArgVar(shift+n-1) = GetArgVar(n-1); //right to left, to avoid over-writing
...
//TODO: (old) should also use optimized code.
SiClosure SiPap_1_1("<Pap:1,1>",Ncode(ApplyPapCode),2,1);
...
// TODO: Is this as terrible as it looks?
SiClosure* getSiPap(unsigned num_early_args, unsigned num_remaining_args) {
  ...
  cout << "**getSiPap(not yet supported): " << num_early_args << ","
       << num_remaining_args << endl;
  NOT_YET;
  //TODO: the following is unreached!
  return makeSiClosure(pap_allocation,"<Pap>",Ncode(ApplyPapCode),
                       num_early_args+1,num_remaining_args);
}
...
//TODO (old) could optimize ApplyOverApp, as frame_size is known!
SiCont SiOverApp1 ("<OverApp1>",Ncode(ApplyOverApp),1);
...
SiCont* getSiOverApp(unsigned frame_size) {
  ...
  cout << "**getSiOverApp(not yet supported): " << frame_size << endl;
  NOT_YET;
  //TODO: following line is not reached
  return makeSiCont("<OverApp>",Ncode(ApplyOverApp),frame_size);
}
...
//TODO: make a test that triggers this bug!
...
Nword builtin_Less(Nword w1,Nword w2) {
  return makeBool (  lessNumTxt(w1,w2));
}

Nword builtin_LessEqual(Nword w1,Nword w2) {
  return makeBool (! lessNumTxt(w2,w1));
}

Nword builtin_Greater(Nword w1,Nword w2) {
  return makeBool (  lessNumTxt(w2,w1));
}

Nword builtin_GreaterEqual(Nword w1,Nword w2) {
  return makeBool (! lessNumTxt(w1,w2));
}
...
Nword builtin_implode(Nword w) {
  string res;
  for (Nword xs = w; getTag(xs) != the_nil_tag;
       xs = getTupleElem (getCon(xs),1)) {
...
Nword builtin_TextIO_closeOut(Nword w) {
  ...
  //not all ostream are ofstream!
  //TODO: will crash if applied to stdout?
...
Nword builtin_TextIO_openIn(Nword w) {
  ...
  //TODO: should throw an exception if cant open file
  assert(ifsp && ifsp->is_open());
...
Nword builtin_Char_LessEqual(Nword w1,Nword w2) {
  return makeBool(getChar(w1) <= getChar(w2));
}

Nword builtin_Char_GreaterEqual(Nword w1,Nword w2) {
  return makeBool(getChar(w1) >= getChar(w2));
}
...
void print_stats(char* tag) {
  ...
  //TODO: Are these stats useful for anything?
...
int main(int argc, char* argv[]) {
  ...
  const unsigned H = 1; // just 1, for the vtable
  ...
  PushContinuation(&SiStopExection);
  ...
  if (debug) {
    print_stats("final");
    cout << "**maxRequiredStaticFrameSize = " << maxRequiredStaticFrameSize
         << endl;
